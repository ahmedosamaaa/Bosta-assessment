        ...
        <div className="flex flex-col items-center w-full md:flex-row md:w-auto md:gap-8">
            {utilityLinks.map(({ to, label }, index) => (
                <NavLink
                    key={index}
                    to={to}
                    className={`relative flex items-center gap-2 text-base font-bold justify-center py-4 md:py-0 border-b border-gray-100 md:border-none w-full md:w-auto hover:text-red-600 transition-colors duration-200 ${
                        index === 0
            ...
            <LanguageSelector className="mt-4 font-bold text-red-600 md:mt-0 text-md " />
        </div>
    );
};
